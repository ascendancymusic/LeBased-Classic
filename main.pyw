...
from tree_hierarchy import TreeLayout, TreeNode  # Add this line
...
class SwitchState(EventDispatcher):
    ...
    background_image_source = StringProperty("graphics/gradient.jpg")  # Default background image
    color1 = ListProperty([0.341, 0.773, 0.714]) # Light blue
    color2 = ListProperty([0.082, 0.596, 0.584]) # Blue
    color3 = ListProperty([0.102, 0.373, 0.478]) # Darker blue
    color4 = ListProperty([0, 0.169, 0.357]) # Dark blue
    color5 = ListProperty([0.341, 0.773, 0.714]) # Light blue
...
class AddToDoScreen(Screen, SwitchState):
    ...
    # Call this function before any operation that interacts with the tasks table
    initialize_database()
    ...
    def on_pre_leave(self, *args):
        # Clear the fields when leaving the screen
        self.ids.title.text = ''
        self.ids.description.text = ''
        self.task_id = None
        
    def on_pre_enter(self, *args):
        # Bind the on_back_button method to the back button press event
        Window.bind(on_keyboard=self.on_back_button)
        self.load_switch_state()

    def on_leave(self, *args):
        # Unbind the event when leaving the screen to avoid multiple bindings
...
class TaskBoxLayout(BoxLayout):
    ...
    switch_state = ObjectProperty(None)  # Reference to SwitchState instance
    bg_color = ListProperty([0.173, 0.129, 0.365, 0.75])  # Initial color
...
class ToDoScreen(Screen, SwitchState):
    ...
    def on_pre_enter(self, *args):
        ...
        # Bind the on_touch_up method to detect swipe gestures
        Window.bind(on_touch_move=self.on_touch_move_window)
        Window.bind(on_touch_up=self.on_touch_up_window)
        self.load_switch_state()

    def on_leave(self, *args):
        # Unbind the events when leaving the screen to avoid multiple bindings
        Window.unbind(on_keyboard=self.on_back_button)
...
class AboutScreen(Screen, SwitchState):
    def on_pre_enter(self, *args):
        # Bind the on_back_button method to the back button press event
        Window.bind(on_keyboard=self.on_back_button)
        self.load_switch_state()  # Load switch state from database when entering the screen
...
class MainScreen(Screen, SwitchState):
    ...
    def update_switch_state(self, state):
        ...
        conn.commit()
        conn.close()
        self.load_switch_state()  # Ensure the background is updated when the switch state changes
    ...
    def on_pre_enter(self, *args):
        self.load_switch_state()  # Ensure the background is updated when entering the screen
